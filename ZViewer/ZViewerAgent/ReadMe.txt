========================================================================
    액티브 템플릿 라이브러리 : ZViewerAgent 프로젝트 개요
========================================================================

사용자가 DLL(동적 연결 라이브러리)을 만드는 데 있어 출발점으로 사용하도록
응용 프로그램 마법사에서 이 ZViewerAgent 프로젝트를 만들었습니다.
...
ZViewerAgent.idl
    ...
    이 파일은 MIDL 컴파일러에 의해 처리되어 다음을 생성합니다.
        C++ 인터페이스 정의 및 GUID 선언 (ZViewerAgent.h)
        GUID 정의                                (ZViewerAgent_i.c)
        형식 라이브러리                                  (ZViewerAgent.tlb)
        마샬링 코드                                 (ZViewerAgent_p.c 및 dlldata.c)

ZViewerAgent.h
    이 파일에는 ZViewerAgent.idl에서 정의된 항목의 C++ 인터페이스가 정의되어 있고
...
StdAfx.h 및 StdAfx.cpp는
    ZViewerAgent.pch라는 이름의 PCH(미리 컴파일된 헤더) 파일과
...
ZViewerAgentps.vcproj
	...
	프록시/스텁 DLL을 빌드하는 데 필요한 dlldata.c, ZViewerAgent_i.c 및
	ZViewerAgent_p.c가 생성됩니다.
